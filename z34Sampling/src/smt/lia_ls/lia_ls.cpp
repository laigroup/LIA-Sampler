...
// constructing literals from strings
...
void ls_solver::build_instance(std::vector<std::vector<int> > &clause_vec) {
    ...
    for (int clause_idx = 0; clause_idx < clause_vec.size(); clause_idx++) {
        if (clause_vec[clause_idx].size() == 1) {  // unit clause, bound lit (x + y + key <= 0)
            ...
            if (l->pos_coff.size() == 0 && l->neg_coff.size() == 1) {  // unit var with negative coeff (-a + key <= 0)
                ...
                if (clause_vec[clause_idx][0] > 0 &&                                        // positive literal
                ...
                } else if (clause_vec[clause_idx][0] < 0 &&                                 // negative literal
                           (l->key - 1) < _tmp_vars[l->neg_coff_var_idx[0]].upper_bound) {  // key < old_upper_bound
                ...
                if (clause_vec[clause_idx][0] > 0 &&                                      // positive literal
                    ...
                    _tmp_vars[l->pos_coff_var_idx[0]].upper_bound = -l->key;              // tightening the upper bound
                           ...
                           (1 - l->key) > _tmp_vars[l->pos_coff_var_idx[0]].low_bound) {  // -k+1 > old_lower_bound
...
int ls_solver::pick_critical_move(__int128_t &best_value) {
    ...
    for (int i = 0; i < unsat_clauses->size(); i++) {
        ...
        for (int l_idx : cl->lia_literals) {
            ...
            // LIA_TRACE(print_literal(tout ,*l));

            if (l->is_equal) {
                if (l_idx < 0) {  // negative literal
                    for (int var_idx : l->pos_coff_var_idx) {
...
// print
void ls_solver::print_formula(std::ostream &out) {
    for (int i = 0; i < _num_clauses; i++) {
...
void ls_solver::print_formula_pbs(std::ostream &out) {
    ...
    for (int lit_idx = 1; lit_idx < _num_lits; lit_idx++) {
        ...
        print_lit_pbs(out, _lits[lit_idx]);
    }
    std::cout << "2 0 1 1 0\n";
}

void ls_solver::print_formula_smt(std::ostream &out) {
...
void ls_solver::print_literal(std::ostream &out, lit &l) {
    if (!l.is_lia_lit) {
...
void ls_solver::print_lit_pbs(std::ostream &out, lit &l) {
    __int128_t degree = l.key;
    for (int i = 0; i < l.pos_coff.size(); i++) {
        degree += l.pos_coff[i];
    }
    out << "num vars: " << _num_vars + 1 << " degree:" << print_128(degree) << " ";
    for (int i = 0; i < l.pos_coff.size(); i++) {
        out << "coff:" << print_128(l.pos_coff[i]) << " var_idx:" << -(l.pos_coff_var_idx[i] + 1) << ";";
    }
    for (int i = 0; i < l.neg_coff.size(); i++) {
        out << "coff:" << print_128(l.neg_coff[i]) << " var_idx:" << l.neg_coff_var_idx[i] + 1 << ";";
    }
    out << "0\n";
}

void ls_solver::print_lit_smt(std::ostream &out, int lit_idx) {
    lit *l = &(_lits[std::abs(lit_idx)]);
    if (l->is_lia_lit) {
        out << "(" << (lit_idx > 0 ? "<=" : ">") << " (- " << _vars[l->pos_coff_var_idx[0]].var_name << " " << _vars[l->neg_coff_var_idx[0]].var_name << ") " << print_128(-l->key) << " ) ";
    } else {
        if (lit_idx > 0) {
            out << _vars[l->delta].var_name << " ";
        } else {
            out << "(" << " not " << _vars[l->delta].var_name << " ) ";
        }
    }
}

void ls_solver::print_mv(std::ostream &out) {
    std::cout << "(model\n";
    for (uint64_t var_idx = 0; var_idx < _num_vars; var_idx++) {
        print_mv_vars(out, var_idx);
    }
    std::cout << ")\n";
}

void ls_solver::print_mv_vars(std::ostream &out, uint64_t var_idx) {
    variable *v = &(_vars[var_idx]);
    __int128_t var_solution = _solution[var_idx];
    out << "  (define-fun " << v->var_name << " () ";
    if (v->is_lia) {
        out << "Int ";
        if (var_solution >= 0) {
            out << print_128(var_solution) << ")\n";
        } else {
            out << "(- " << print_128(-var_solution) << "))\n";
        }
    } else {
...
void ls_solver::up_bool_vars() {
    for (variable &var : _resolution_vars) {                                      // 从 resolution var 到 reduced var 这一步被约简掉的变量赋值
        if (var.is_lia && name2var.find(var.var_name) == name2var.end()) {        // if it is an lia var and it is not in the formula
            ...
            if (var.low_bound > 0) {
                _solution[var_idx] = var.low_bound;
            } else if (var.upper_bound < 0) {
                _solution[var_idx] = var.upper_bound;
            } else {
                _solution[var_idx] = 0;
            }
        }
...
void ls_solver::choose_value_for_pair() {
    ...
    for (int pair_idx = 0; pair_idx < pair_x->size(); pair_idx++) {
        ...
        LIA_TRACE(
            tout << pair_idx << " pair:\n";
            tout << "original_var_x: " << original_var_x->var_name << "\n";
            tout << "original_var_y: " << original_var_y->var_name << "\n";
            tout << "z_value: " << print_128(z_value) << "\n";
            tout << "upper_y: " << print_128(upper_y) << "\n";
            tout << "lower_y: " << print_128(lower_y) << "\n";
            tout << "upper: " << print_128(upper) << "\n";
            tout << "lower: " << print_128(lower) << "\n";
            tout << "new_x_value: " << print_128(new_x_value) << "\n";
            tout << "new_y_value: " << print_128(new_x_value - z_value) << "\n";);
    }
}
...
bool ls_solver::local_search() {
    ...
    int no_improve_cnt = 0;  // non_improve_step
    int flipv;
    __int128_t change_value = 0;
    ...
    for (_step = 1; _step < _max_step; _step++) {
        ...
        // only when use_swap and 1% probabilty and
        // |sat_clauses_with_false_literal| is more than 1, do the swap from small weight
        if (!use_swap_from_from_small_weight || mt() % 100 < 99 || sat_clause_with_false_literal->size() == 0) {
            bool time_up_bool = (no_improve_cnt_bool * _lit_in_unsat_clause_num >
                                 5 * _bool_lit_in_unsat_clause_num)  // non_improve_steps > L * Pb
                                || (unsat_clauses->size() <= 20);
            bool time_up_lia = (no_improve_cnt_lia * _lit_in_unsat_clause_num >  // non_improve_steps > L * Pi
