...
void context::build_core_clauses() {
    if (!m_b_internalized_stack.empty()) {
        ...
        origin_lit_num = m_b_internalized_stack.size();
        partial_LS_phase_vec.resize(origin_lit_num);
        complete_LS_phase_vec.resize(origin_lit_num);
        core_clauses = clauses_vec;
        clauses_vec.clear();  // copy the clauses into core_clauses
    }
}

void context::expr_bool_var_map(nra::ls_solver* solver) {
    std::string myString;
    if (!m_b_internalized_stack.empty()) {
        uint64_t sz = m_b_internalized_stack.size();
#ifdef LS_DEBUG
        ...
        for (uint64_t i = 0; i < sz; i++) {
            ...
            }  // Store expressions corresponding to the boolean variable inside string
            myString = ss.str();
#ifdef LS_DEBUG
            std::cout << myString << "\n";
#endif
...
void context::read_lits(nia::ls_solver* solver, bool use_all_lits) {
    std::string myString;
    if (!m_b_internalized_stack.empty()) {
        uint64_t sz = (use_all_lits) ? m_b_internalized_stack.size() : origin_lit_num;
#ifdef LS_DEBUG
        std::cout << sz << "\n";
#endif
        solver->make_lits_space(sz);
...
void context::expr_bool_var_map(lia::ls_solver* solver) {
    ...
    if (!m_b_internalized_stack.empty()) {
        ...
        int if_var_num = 0;
#ifdef LIA_DEBUG
        std::vector<std::string> debug_myString;
#endif
        ...
        for (uint64_t i = 0; i < sz; i++) {
            ...
            solver->build_lits(myString);
#ifdef LIA_DEBUG
            debug_myString.push_back(myString);
#endif
        }
#ifdef LIA_DEBUG
        LIA_TRACE(
        ...
        LIA_TRACE(display_expr_bool_var_map(tout););
    }
}
...
void context::expr_bool_var_map(sampler::ls_sampler* sampler) {
    ...
    if (!m_b_internalized_stack.empty()) {
        ...
        for (uint64_t i = 0; i < sz; i++) {
            expr* n = m_b_internalized_stack.get(i);  // The expr corresponding to the lit

            bool_var v = get_bool_var_of_id(n->get_id());  // Get the Boolean variable corresponding to the literal ID.
            literal l_curr = get_literal(n);               // An integer value representing the literal
            std::stringstream ss;
            ss << l_curr.var() << " ";  // var (int)

            // Converting four structures to CNF, including
            // 1) Boolean equations
            // 2) or clauses
            // 3) ite clauses
            // 4) conventional form
            if (to_app(n)->get_decl()->get_name() == "=" && m.is_bool(to_app(n)->get_arg(0))) {  // a = b && a is bool sort
                expr* eq_term_a = to_app(n)->get_arg(0);                                         // left side of the equation
                expr* eq_term_b = to_app(n)->get_arg(1);                                         // right side of the equation
                bool_var A, a, b, c;                                                             // int
                if (to_app(eq_term_a)->get_decl()->get_name() == "not") {                        // negative literal
                    a = -get_bool_var_of_id(to_app(eq_term_a)->get_arg(0)->get_id());            // - bool var
                } else {
                    a = get_bool_var_of_id(eq_term_a->get_id());  // bool var
                }
                if (to_app(eq_term_b)->get_decl()->get_name() == "not") {
                    b = -get_bool_var_of_id(to_app(eq_term_b)->get_arg(0)->get_id());
                } else {
                ...
                A = l_curr.var();  // Variable corresponding to literal

                // Boolean Equation Conversion to Multiple Clauses
                std::vector<int> clause_tmp;
                clause_tmp.push_back(-A);
                clause_tmp.push_back(-a);
                clause_tmp.push_back(b);  //-A or -a or b (A and a --> b)
                clauses_vec.push_back(clause_tmp);
                clause_tmp.clear();
                clause_tmp.push_back(-A);
                clause_tmp.push_back(a);
                ...
                std::vector<int> clause_tmp;
                clause_tmp.push_back(-l_curr.var());
                for (unsigned j = 0; j < to_app(n)->get_num_args(); j++) {  // Iterate over the parsed literal
                    expr* or_term = to_app(n)->get_arg(j);
                    std::vector<int> clause_tmp_1;
                    clause_tmp_1.push_back(l_curr.var());
                    if (to_app(or_term)->get_decl()->get_name() == "not") {
                        bool_var v1 = get_bool_var_of_id(to_app(or_term)->get_arg(0)->get_id());
                        clause_tmp.push_back(-v1);   // -A or -v1 (A --> -v1)
                        clause_tmp_1.push_back(v1);  // A or v1 (-A --> v1)
                    } else {
                        bool_var v1 = get_bool_var_of_id(or_term->get_id());
                        clause_tmp.push_back(v1);     // -A or v1 (A --> v1)
                        clause_tmp_1.push_back(-v1);  // A or -v1 (-A --> -v1)
                    }
                    clauses_vec.push_back(clause_tmp_1);  // When a clause is false, there must be individual literals in the clause that are false
                }
                clauses_vec.push_back(clause_tmp);  // When a clause is true, there must be at least one literal in the clause that is true
                ss << "or new_var" << new_var_num++;
            } else if (to_app(n)->get_decl()->get_name() == "if") {
                expr* if_term_a = to_app(n)->get_arg(0);
                expr* if_term_b = to_app(n)->get_arg(1);
                ...
                clause_tmp.push_back(-A);
                clause_tmp.push_back(a);
                clause_tmp.push_back(c);  //-A or a or c (A and -a --> c)
                clauses_vec.push_back(clause_tmp);
                clause_tmp.clear();
                clause_tmp.push_back(A);
                clause_tmp.push_back(-a);
                ...
                clause_tmp.push_back(A);
                clause_tmp.push_back(-b);
                clause_tmp.push_back(-c);  // A or -b or -c (-A --> -b or -c)
                clauses_vec.push_back(clause_tmp);
                ss << "if if_var" << if_var_num++;
            } else {
                smt::display(ss, l_curr, m, m_bool_var2expr.data());
...
void context::log_stats() {
    ...
    if (bin_lemmas > 0) strm << "/" << bin_lemmas << " ";
