...
std::ostream& operator<<(std::ostream& out, model_value_dependency const& src) {
    if (src.is_fresh_value())
        return out << "fresh!" << src.get_value()->get_idx();
    else
        return out << "#" << src.get_enode()->get_owner_id();
}
...
void model_generator::mk_bool_model(sampler::ls_sampler* sampler) {
    unsigned sz = m_context->get_num_b_internalized();
    for (unsigned i = 0; i < sz; i++) {
        expr* p = m_context->get_b_internalized(i);
        if (is_uninterp_const(p)) {
            ...
            std::string var_name = var_name_stream.str();
            std::string var_value;
            sampler->print_var_solution(var_name, var_value);
            expr* v = var_value == "1" ? m.mk_true() : m.mk_false();
            m_model->register_decl(d, v);
        }
    }
...
*/
void model_generator::mk_value_procs(obj_map<enode, model_value_proc*>& root2proc, ptr_vector<enode>& roots, ptr_vector<model_value_proc>& procs) {
...
void model_generator::mk_values() {
    ...
    TRACE("sorted_sources", for (source const& curr : sources) {
    ...
    for (enode* n : m_context->enodes()) {
        if (is_uninterp_const(n->get_expr())) {
            ...
            if (m_hidden_ufs.contains(d))
                continue;
            expr* val = get_value(n);
            TRACE("model", tout << "first register decl!\n";);
            m_model->register_decl(d, val);
        }
    }
}

void model_generator::mk_values_ls(sampler::ls_sampler* sampler) {
    arith_factory a_fac = arith_factory(m);
    rational num;
    for (enode* n : m_context->enodes()) {
        app* val;
        if (is_uninterp_const(n->get_expr())) {
            ...
            sampler->print_var_solution(var_name, var_value);
            num = rational(var_value.c_str()).get_rational();
            if (num > rational("170141183460469231731687303715884105727") || num < rational("-170141183460469231731687303715884105727")) {
                std::cout << "large num is: " << num << "\n";
                throw default_exception("large num");
            }
            val = a_fac.mk_num_value(num, true);  // 只要在此处将val赋值便可以了
            register_value(val);
...
bool model_generator::include_func_interp(func_decl* f) const {
    ...
    if (fid == null_family_id)
        return !m_hidden_ufs.contains(f);
...
void model_generator::mk_func_interps() {
    ...
    for (unsigned i = 0; i < sz; i++) {
        ...
        if (num_args == 0 && include_func_interp(f)) {
            TRACE("model", tout << "second register decl!\n";);
            ...
            TRACE("model", tout << "insert new entry for:\n"
                                << mk_ismt2_pp(n->get_expr(), m) << "\nargs: ";
                  for (unsigned i = 0; i < num_args; i++) { tout << "#" << n->get_arg(i)->get_owner_id() << " "; } tout << "\n"; for (expr* arg : args) { tout << mk_pp(arg, m) << " "; } tout << "\n"; tout << "value: #" << n->get_owner_id() << "\n"
...
void model_generator::register_macros() {
    ...
    for (unsigned i = 0; i < num; i++) {
        ...
        TRACE("model", tout << f->get_name() << " = " << mk_pp(v, m) << "\n";);
        m_model->register_decl(f, fi);
    }
}
...
proto_model* model_generator::mk_model_ls(lia::ls_solver* solver, size_t idx) {
    init_model();
    register_existing_model_values();  // 注册已经存在的model 值，此处不会调用register_value 函数
    mk_bool_model(solver);
    mk_values_ls(solver);  // 此处会调用register_value函数 (first register)
    mk_func_interps(); // (second register)
    finalize_theory_models();
    register_macros();
    return m_model.get();
}

proto_model* model_generator::mk_model_sampling(sampler::ls_sampler* sampler) {
    init_model();
    register_existing_model_values();  // 注册已经存在的model 值，此处不会调用register_value 函数
    mk_bool_model(sampler);
    mk_values_ls(sampler);  // 此处会调用register_value函数 (first register)
    mk_func_interps(); // (second register)
    finalize_theory_models();
    register_macros();
    return m_model.get();
