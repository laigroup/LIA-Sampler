...
context::context(ast_manager& m, smt_params& p, params_ref const& _p) : m(m),
                                                                        m_fparams(p),
                                                                        m_params(_p),
                                                                        m_setup(*this, p),
    ...
    m_ls_sampler = new sampler::ls_sampler(m_fparams.m_random_seed, m_fparams.m_input_file, m_fparams.m_output_file);
...
class add_eq_trail : public trail {
   ...
   public:
    add_eq_trail(context& ctx, enode* r1, enode* n1, unsigned r2_num_parents) : ctx(ctx),
                                                                                m_r1(r1),
                                                                                m_n1(n1),
...
void context::add_eq(enode* n1, enode* n2, eq_justification js) {
    ...
    try {
        ...
        TRACE("add_eq", tout << "merging: #" << r1->get_owner_id() << " #" << r2->get_owner_id() << " n1: #" << n1->get_owner_id() << "\n";);
...
bool context::is_diseq_slow(enode* n1, enode* n2) const {
    ...
    for (enode* parent : enode::parents(n1)) {
        if (parent->is_eq() && is_relevant(parent->get_expr()) && get_assignment(enode2bool_var(parent)) == l_false &&
            ...
            TRACE("is_diseq_bug", tout << "parent: #" << parent->get_owner_id() << ", parent->root: #" << parent->get_root()->get_owner_id() << " assignment(parent): " << get_assignment(enode2bool_var(parent)) << " args: #" << parent->get_arg(0)->get_owner_id() << " #" << parent->get_arg(1)->get_owner_id() << "\n";);
            return true;
        }
    }
    return false;
        ...
        if (parent->is_eq()) {
            ...
            if (get_assignment(bv) == l_false) {
                ...
                TRACE("push_new_th_diseqs",
                      ...
                      tout << "lhs: #" << lhs->get_owner_id() << ", rhs: #" << rhs->get_owner_id() << ", lhs->root: #" << lhs->get_root()->get_owner_id() << ", rhs->root: #" << rhs->get_root()->get_owner_id() << ", r: #" << r->get_owner_id() << ", r->root: #" << r->get_root()->get_owner_id() << "\n";);
...
bool context::can_propagate() const {
    return m_qhead != m_assigned_literals.size() ||
...
void context::dec_ref(literal l) {
    if (track_occs() && m_lit_occs[l.index()] > 0) m_lit_occs[l.index()]--;
}

void context::inc_ref(literal l) {
    if (track_occs()) m_lit_occs[l.index()]++;
}
...
void context::del_inactive_lemmas1() {
    ...
    for (; i < end_at; i++) {
        ...
        if (can_delete(cls)) {
            TRACE("del_inactive_lemmas", tout << "deleting: "; display_clause(tout, cls); tout << ", activity: " << cls->get_activity() << "\n";);
            del_clause(true, cls);
            num_del_cls++;
        } else {
            m_lemmas[j] = cls;
...
class case_split_insert_trail : public trail {
   ...
   public:
    case_split_insert_trail(context& ctx, literal l) : ctx(ctx),
                                                       l(l) {
    }
    void undo() override {
...
void context::mk_th_case_split(unsigned num_lits, literal* lits) {
    ...
    if (!m_fparams.m_theory_case_split) {
        ...
        TRACE("theory_case_split", tout << "tracking case split literal set { "; for (unsigned i = 0; i < num_lits; ++i) { tout << lits[i].index() << " "; } tout << "}" << std::endl;);
    }
}
...
bool context::propagate_th_case_split(unsigned qhead) {
    ...
    for (; qhead < assigned_literal_end; ++qhead) {
        ...
        for (vector<literal_vector>::const_iterator it = case_split_sets.begin(); it != case_split_sets.end(); ++it) {
            ...
            TRACE("theory_case_split", tout << "found case split set { "; for (literal_vector::iterator set_it = case_split_set.begin(); set_it != case_split_set.end(); ++set_it) { tout << set_it->index() << " "; } tout << "}" << std::endl;);
...
lbool context::setup_and_check(bool reset_cancel) {
    ...
    if (m_fparams.m_sampling) {
        expr_bool_var_map(m_ls_sampler);
        m_ls_sampler->build_instance(clauses_vec);
        m_ls_sampler->ls_sampling();
        if (m_ls_sampler->_best_found_cost == 0){
            try{
                m_model_generator->reset();
    ...
    if (!theory_assumptions.empty()) {
        ...
        if (is_NIA) {
            ...
            build_core_clauses();
            // print_clauses_vec(std::cout, core_clauses);
            ...
            if (!m_fparams.m_use_ls) {
                ...
                if (m_lia_ls_solver->best_found_cost == 0) {
                    try {
                        m_model_generator->reset();
                        m_proto_model = m_model_generator->mk_model_ls(m_lia_ls_solver); // 构造 ls 生成的部分模型
                    } catch (default_exception& ex) {
                        return check_finalize(l_undef);
                    }
                    return check_finalize(l_true); // 后面还需要构造
                }
                return check_finalize(l_undef);
            }
        }  // LIA case
            ...
            if (!m_fparams.m_use_ls) {
                ...
                // print_clauses_vec(std::cout, clauses_vec);
    ...
    switch (into_rephase_cnt++ % 4) {
        case 0:
            for (expr* n : m_b_internalized_stack) {
                m_bdata[get_bool_var(n)].m_phase = ((m_random() % 2) == 0);
            }
            break;
        case 1:
            for (expr* n : m_b_internalized_stack) {
                m_bdata[get_bool_var(n)].m_phase = false;
            }
            break;
        case 2:
            for (expr* n : m_b_internalized_stack) {
...
void context::rephase_partial_ls() {
    ...
    if ((pick_rand -= 500) < 0) {
        for (uint64_t i = 0; i < partial_ls_vector_sz; i++) {
            if (partial_LS_phase_vec[i] != l_undef) {
                m_bdata[i].m_phase = (partial_LS_phase_vec[i] == l_true) ? true : false;
            } else {
                m_bdata[i].m_phase = ((m_random() % 2) == 0);
            }
        }
    }
    ...
    // reverse
        ...
        for (uint64_t i = 0; i < sz; i++) {
            m_bdata[i].m_phase = !m_bdata[i].m_phase;
        }
    }
    // random
    else if ((pick_rand -= 100) < 0) {
    ...
    // all true
        ...
        for (uint64_t i = 0; i < sz; i++) {
            m_bdata[i].m_phase = true;
        }
    }
    // all false
    else if ((pick_rand -= 100) < 0) {
        for (uint64_t i = 0; i < sz; i++) {
            m_bdata[i].m_phase = false;
        }
    }
    // remain
    else {
...
void context::rephase_complete_ls() {
    ...
    if ((pick_rand -= 500) < 0) {
        for (uint64_t i = 0; i < complete_ls_vector_sz; i++) {
            if (complete_LS_phase_vec[i] != 0) {
                m_bdata[i].m_phase = (complete_LS_phase_vec[i] > 0) ? true : false;
            } else {
                m_bdata[i].m_phase = ((m_random() % 2) == 0);
            }
        }
...
void context::call_ls(const std::vector<std::vector<int>>& cl_vec, uint64_t ls_max_step, bool complete_ls, int& ls_best_found_cost, bool record_frequency) {
    ...
    if (!has_unidentified_lits) {
        ...
        if (complete_ls) {
            save_phase_complete(nia_ls_solver);
        }
        if (record_frequency) {
            modify_activity_by_LS(nia_ls_solver);
        }
...
lbool context::bounded_search() {
    ...
    if (is_NIA) {
        if (restart_cnt >= restart_threshold) {
            ...
            if (complete_ls_step < 300000) {
                complete_ls_step += 10000;
            }
        }
    }
    restart_cnt++;
        ...
        while (!propagate()) {
            ...
            if (is_NIA) {
                if (!after_resove_with_false_clause) {
                    ...
                    after_resolve_decide_cnt = 0;
                    if (after_resolve_decide_gap >= 16) {
                        after_resolve_decide_gap = (after_resolve_decide_gap >> 2);
                    }
            ...
            if (after_resove_with_false_clause) {
                if ((after_resolve_decide_cnt++) >= after_resolve_decide_gap) {
                    ...
                    if (check_original_clauses(core_clauses)) {
                        ...
                        if (partial_ls_step > 10000) {
                            partial_ls_step -= 10000;
                        }
                    }
                }
            }
...
bool context::resolve_conflict() {
    ...
    if (m_conflict_resolution->resolve(m_conflict, m_not_l)) {
        ...
        TRACE("resolve_conflict_bug", tout << "m_scope_lvl: " << m_scope_lvl << ", new_lvl: " << new_lvl << ", lemma_intern_lvl: " << m_conflict_resolution->get_lemma_intern_lvl() << "\n"; tout << "num_lits: " << num_lits << "\n"; for (unsigned i = 0; i < num_lits; i++) {
                      literal l = lits[i];
                      tout << l << " ";
                      display_literal_smt2(tout, l);
        ...
        reset_cache_generation();
        TRACE("resolve_conflict_bug", tout << "AFTER m_scope_lvl: " << m_scope_lvl << ", new_lvl: " << new_lvl << ", lemma_intern_lvl: " << m_conflict_resolution->get_lemma_intern_lvl() << "\n"; tout << "num_lits: " << num_lits << "\n"; for (unsigned i = 0; i < num_lits; i++) {
                      literal l = lits[i];
                      tout << l << " ";
        ...
        TRACE("context_lemma", tout << "new lemma: "; literal_vector v(num_lits, lits); std::sort(v.begin(), v.end()); for (unsigned i = 0; i < num_lits; i++) {
...
void context::extract_true_clause(std::vector<std::vector<int>>& new_vec) {
    ...
    for (int l = 0; l < origin_lit_num; l++) {
        cls_tmp.clear();
        if (get_assignment(l) == l_undef || appear_sat_in_clause[l] == false) {
            cls_tmp.push_back(0);
        }
        if (get_assignment(l) == l_true) {
            cls_tmp.push_back(l);
        } else if (get_assignment(l) == l_false) {
            cls_tmp.push_back(-l);
        }
        new_vec[l] = cls_tmp;
    }
