...
struct two_ex_probe : public probe {
    struct found {};
    ...
    struct proc {
        ...
        void operator()(app* n) {
            if (n->get_num_args() == 0 && m.is_value(n)) {
                std::stringstream var_value;
                var_value << mk_pp(n, m);
                rational num = rational(var_value.str().c_str()).get_rational();
                int n = num.get_int32();
                if (n > maxint) {
                    maxint = n;
                }
...
probe* mk_two_ex_probe() {
    return alloc(two_ex_probe);
}
...
static tactic* mk_ls_smt_tactic(ast_manager& m, unsigned rs, unsigned ls_time) {
    params_ref solver_p;
    solver_p.set_bool("use_ls", true);
    solver_p.set_uint("random_seed", rs);
    solver_p.set_uint("ls_time", ls_time);
    return annotate_tactic("ls-tactic", using_params(mk_smt_tactic_using(m, false), solver_p));
}

static tactic* mk_sampling_smt_tactic(ast_manager& m, unsigned num, unsigned seed, char const* input, char const* output, unsigned lambda, double time_limt) {
    params_ref solver_p;
    // solver_p.set_bool("sampling", true);
    // solver_p.set_double("time_limt", time_limt);
    // solver_p.set_str("input_file", input);
    // solver_p.set_str("output_file", output);
    // solver_p.set_uint("seed", seed);
    // solver_p.set_uint("lambda", lambda);

    return annotate_tactic("sampling-tactic", using_params(mk_smt_tactic_using(m, false), solver_p));
}
...
// dedicate for ring
static tactic* mk_ring_tactic(ast_manager& m) {
    return annotate_tactic(
        "ring-tactic",
        and_then(
            // fail_if_not(mk_and(mk_le(mk_num_bool_consts_probe(),mk_const_probe(static_cast<double>(1))),mk_two_ex_probe())),
            fail_if_not(mk_and(mk_and(mk_le(mk_num_bool_consts_probe(), mk_const_probe(static_cast<double>(1))), mk_has_ite_probe()), mk_two_ex_probe())),
            //  or_else(
            //      try_for(mk_smt_tactic(m),50000),
            mk_unsat_tactic(),
            mk_fail_if_undecided_tactic()));
}
...
tactic* mk_qflia_tactic(ast_manager& m, params_ref const& p) {
    ...
    tactic* st;
    if (first_sampling) {
        std::cout << "sampling tactic\n";
        st = using_params(  // ls sampling
            and_then(
                mk_preamble_tactic(m),
                using_params(mk_simplify_tactic(m), lhs_p),
                sampling_tactic),
