...
class binary_tactical : public tactic {
   ...
   public:
    binary_tactical(tactic* t1, tactic* t2) : m_t1(t1),
                                              m_t2(t2) {
        SASSERT(m_t1);
        SASSERT(m_t2);
    }
    ...
    void operator()(goal_ref const& in, goal_ref_buffer& result) override {
        ...
        if (finished_id == UINT_MAX) {
            switch (ex_kind) {
                ...
                case TACTIC_EX:
                    throw tactic_exception(std::move(ex_msg));
                default:
...
class repeat_tactical : public unary_tactical {
   ...
   public:
    repeat_tactical(tactic* t, unsigned max_depth) : unary_tactical(t),
...
class annotate_tactical : public unary_tactical {
   ...
   public:
    annotate_tactical(char const* name, tactic* t) : unary_tactical(t), m_name(name) {}
...
class cond_tactical : public binary_tactical {
   ...
   public:
    cond_tactical(probe* p, tactic* t1, tactic* t2) : binary_tactical(t1, t2),
                                                      m_p(p) {
        SASSERT(m_p);
    }
